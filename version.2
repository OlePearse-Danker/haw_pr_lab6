...

{
    ...
    char content[1024]; // creating array to store the file content

    // reading from txt file and storing it in this array

    if (fptr != NULL) //if file opening is successful -> opening was successful
    {
        while (fgets(content, 1024, fptr)) // using fgets function to read the content of a file (fptr) and store content in "content" //fgets can only read the first sentence at a time -> while loop to read everything
        {
    ...
    // Parse argument argv[3] to integer value key

    // Get size of arg
    size_t arg_len = strlen(argv[3]);

    // If size is 0, quit!
    if (arg_len == 0) exit(EXIT_FAILURE);

    // Allocate memory with enough room for string + '\0'
    char* key_as_str = malloc((arg_len + 1) * sizeof(*key_as_str));

    // Check for NULL ptr, if memory cannot be allocated
    if (key_as_str == NULL) exit(EXIT_FAILURE);

    // Copy argument to key_as_string, is null terminated
    strcpy(key_as_str, argv[3]);

    // Convert string to long and store as an integer
    // Returns 0 if conversion fails
    // endptr can be used for error checking
    char* endptr = NULL;
    int key = strtol(key_as_str, endptr, 10);

    // Free allocated memory for key_as_str buffer
    free(key_as_str);



    int verschiebung_result = key;
